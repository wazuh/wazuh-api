...
/**
 * @api {put} /experimental/active-response/:agent_id Run an AR command in the agent
 ...
 *
 * @apiExample {curl} Example usage:
 *     curl -u foo:bar -k -X PUT -d '{"command":"restart-ossec0", "arguments": ["-", "null", "(from_the_server)", "(no_rule_id)"]}' -H 'Content-Type:application/json' "https://127.0.0.1:55000/experimental/active-response/001?pretty"
 */
router.put('/active-response/:agent_id', function(req, res) {
    logger.debug(req.connection.remoteAddress + " PUT /experimental/active-response/:agent_id");

    var data_request = {'function': '/PUT/experimental/active-response/:agent_id', 'arguments': {}};
