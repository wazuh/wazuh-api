 ...
 *
 * @apiExample {curl} Example usage:
 *     curl -u foo:bar -X POST -H 'Content-type: application/xml' -d @rules.xml "https://127.0.0.1:55000/rules?pretty"
 *
 */
router.post('/update/:file_name', cache(), function(req, res) {
    logger.debug(req.connection.remoteAddress + " POST /rules/update/:file_name");

    req.apicacheGroup = "agents";

    var data_request = {'function': 'POST/rules/update/:file_name', 'arguments': {}};
